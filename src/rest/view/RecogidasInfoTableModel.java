...
public class RecogidasInfoTableModel extends AbstractTableModel {
	...
	public RecogidasInfoTableModel() {
		this.pedidos = new ArrayList<>();
	}
	
	public RecogidasInfoTableModel(List<Pedido> pedidos) {
		this.pedidos = pedidos;
		for(Pedido p: pedidos) {
			for(Plato plat: p.getPlatos()) {
				if(plat.getStateCocina() && !plat.getStateCamarero()) {
					platosList.add(plat);
