...
public class CocinaWindow extends JPanel implements GestionObserver {
	...
	public CocinaWindow(Controller ctrl) {
		this.ctrl = ctrl;
	...
	private void initGUI() {
		...
		JLabel listoText = new JLabel(" marcar linea seleccionada como lista");
		...
		this.mesa = new PedidosInfoTableModel(this.pedidosMesa);
		...
		this.delivery = new PedidosInfoTableModel(this.pedidosDelivery);
	...
	public void onPedAdded(Pedido ped) {
		if(ped.getType().equals("Mesa")) {
			...
			this.mesa = new PedidosInfoTableModel(pedidosMesa);
			mesaTable.setModel(this.mesa);
			mesaTable.repaint();
		}
		else {
			this.pedidosDelivery.add(ped);
			this.delivery = new PedidosInfoTableModel(pedidosDelivery);
