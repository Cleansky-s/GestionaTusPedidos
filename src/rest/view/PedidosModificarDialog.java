...
public class PedidosModificarDialog extends JDialog implements GestionObserver {
...
private Controller ctrl;
	...
	private PlatosTableModel platos;
	...
	public PedidosModificarDialog(Frame parent, Controller ctrl) {
		super(parent, true);
		this.ctrl = ctrl;
	...
	private void initGUI() {
		...
		consultarButton.addActionListener((e)->{
			...
			String selectedOption = (String) comboBoxOptions.getSelectedItem();
			 ...
			 for(Plato p: platos) {
				 ...
				 dataTableModel.insertRow(i, new String[] {p.getNombre() , p.getCant().toString(), estado});
				 i++;
			 }
		...
		dataTableModel.setColumnIdentifiers(platos.getHeader());
		...
		*/
	    ...
	    if (selection == JOptionPane.OK_OPTION){
	    	...
	    	List<Plato> platos = ped.getPlatos();
	    	
	    	for(int j = 0; j < dataTableModel.getRowCount(); j++) {
	    		String cantStr = dataTableModel.getValueAt(j, 1).toString();
	    		Integer cant = Integer.parseInt(cantStr);
	    		platos.get(j).setCant(cant);
	    	}
